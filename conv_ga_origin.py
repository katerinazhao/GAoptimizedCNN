...
# Step 0: get data
file_path = "/Users/katerina/Library/Mobile Documents/com~apple~CloudDocs/Documents/CNNpaperclone/KS11-3.csv"  
...
feats_col = ['momentum', 'stochastic_K', 'RSI', 'MACD', 'LW', 'A/D Oscillator', 'CCI']
...
train_data = train_data[feats_col]
test_data = test_data[feats_col]
...
# rolling time window to make training dataset
train_sequences = []
train_labels = []
...
# rolling time window to make test dataset
test_sequences = []
test_labels = []
...
# convert to torch tensor (use gpu)
...
def train_and_eval(num_kernel_conv1, num_kernel_conv2, kernel_size_conv1, kernel_size_conv2, kernel_size_pooling, print_accuracy = False, return_history = False):
    ...
    model = TimeSeriesCNN(input_length = TIME_WINDOW_SIZE, num_kernel_conv1=num_kernel_conv1, num_kernel_conv2=num_kernel_conv2, kernel_size_conv1=kernel_size_conv1, kernel_size_conv2=kernel_size_conv2, kernel_size_pooling=kernel_size_pooling)
    ...
    try: # if params is valid, train and eval
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(0, train_sequences_tensor.shape[0],batch_size):
                ...
                # 0-1 classification, down <= 0.5, up > 0.5
                predicted = (outputs.data > 0.5).to(int)

                # compute num of correct pred in training
                ...
                # compute average loss in one training epoch
                avg_loss_per_epoch += loss.data
                
                # reset gradient and back prop
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        ...
        # compute fitness, use test accuracy as fitness of this gene
...
def fitness_func(ga, solution, solution_idx):
    ...
    kernel_size_conv2 = solution[9:16]
    kernel_size_pooling = solution[16]
...
# step 8 find best pop with GAs
'''
params range:
num_kernel_conv1: 1-64
num_kernel_conv2: 1-64
kernel_size_conv1: 1-31
kernel_size_conv2: 1-31
kernel_size_pooling: 1-31
'''
def flatten(a):
    for each in a:
...
#define min and max value of each variable
gene_space = list(flatten([{'low': 1, 'high': 64}, 
              {'low': 1, 'high': 64}, 
              [{'low': 1, 'high': 32}] *7, 
              [{'low': 1, 'high': 32}] *7, 
              {'low': 1, 'high': 32}, 
