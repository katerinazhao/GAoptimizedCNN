...
def train_and_eval(num_kernel_conv1, num_kernel_conv2, kernel_size_conv1, kernel_size_conv2, kernel_size_pooling, generation, individual, print_accuracy=False, return_history=False):
    ...
    try:
        ...
        plt.legend()
        plt.savefig(f'/Users/katerina/Documents/GAoptimizedCNN/train_3rd/gen_{generation}_ind_{individual}_accuracy.png')
        plt.close()
...
def fitness_func(ga, solution, solution_idx):
    ...
    # fetch hyper params
    num_kernel_conv1_bin = solution[0:6]  # Binary list
    num_kernel_conv2_bin = solution[6:12]  # Binary list
    
    # Extract kernel sizes for conv1
    kernel_size_conv1_bin = [solution[12 + i*5:17 + i*5] for i in range(7)]
    
    # Extract kernel sizes for conv2
    kernel_size_conv2_bin = [solution[47 + i*5:52 + i*5] for i in range(7)]
    
    # Extract pooling size
    #kernel_size_pooling_bin = solution[82:87]  # Binary list

    num_kernel_conv1 = int(''.join(num_kernel_conv1_bin.astype(str)), 2)
    num_kernel_conv2 = int(''.join(num_kernel_conv2_bin.astype(str)), 2)
    kernel_size_conv1 = [int(''.join(x.astype(str)), 2) for x in kernel_size_conv1_bin]
    kernel_size_conv2 = [int(''.join(x.astype(str)), 2) for x in kernel_size_conv2_bin]
    #kernel_size_pooling = int(''.join(kernel_size_pooling_bin.astype(str)), 2)
    kernel_size_pooling = 2

    if num_kernel_conv1 == 0 or num_kernel_conv2 == 0 : #kernel_size_pooling == 0
    
        return 0 # zero fitness
    ...
    for x in kernel_size_conv1:
        if x == 0:
            return 0
    print("kernel_size_conv1:", kernel_size_conv1)

    for x in kernel_size_conv2:
        if x == 0:
            return 0
    print("kernel_size_conv2:", kernel_size_conv2)

    
    # Debugging: print solution and num_kernel_conv1_bin
    print("solution:", solution)
    print("num_kernel_conv1_bin:", num_kernel_conv1)
...
def on_generation(ga_instance):
    print(f"on_generation called with generation: {ga_instance.generations_completed}")
...
def save_plot_and_params(ga_instance):
    ...
    params_df.to_csv(f'/Users/katerina/Documents/GAoptimizedCNN/train_3rd/params_gen_{generation}.csv', index=False)

    # Save to Excel
    params_df.to_excel(f'/Users/katerina/Documents/GAoptimizedCNN/train_3rd/params_gen_{generation}.xlsx', index=False)
    # 保存图形
    plt.figure()
    ...
    plt.legend()
    plt.savefig(f'/Users/katerina/Documents/GAoptimizedCNN/train_3rd/fitness_plot_gen_{ga_instance.generations_completed}.png')
    plt.close()
    ...
    params_df = pd.DataFrame(params)
    params_df.to_csv(f'/Users/katerina/Documents/GAoptimizedCNN/train_3rd/params_gen_{ga_instance.generations_completed}.csv', index=False)
...
def encode_gene(num_kernel_conv1, num_kernel_conv2, kernel_size_conv1, kernel_size_conv2, kernel_size_pooling):
    ...
    for size in kernel_size_conv1:
        gene += list(map(int, bin(size - 1)[2:].zfill(5)))
...
def decode_gene(gene):
    num_kernel_conv1_bin = gene[0:6]
    num_kernel_conv2_bin = gene[6:12]
    
    kernel_size_conv1_bin = [gene[12 + i*5:17 + i*5] for i in range(7)]
    kernel_size_conv2_bin = [gene[47 + i*5:52 + i*5] for i in range(7)]
    
    kernel_size_pooling_bin = gene[82:87]
    
    num_kernel_conv1 = int("".join(map(str, num_kernel_conv1_bin)), 2) + 1
    num_kernel_conv2 = int("".join(map(str, num_kernel_conv2_bin)), 2) + 1
    kernel_size_conv1 = [int("".join(map(str, bin_list)), 2) + 1 for bin_list in kernel_size_conv1_bin]
    kernel_size_conv2 = [int("".join(map(str, bin_list)), 2) + 1 for bin_list in kernel_size_conv2_bin]
    #kernel_size_pooling = int("".join(map(str, kernel_size_pooling_bin)), 2) + 1
    kernel_size_pooling = 2
...
for _ in range(sol_per_pop):
    individual = np.random.randint(2, size=82) #size=87
...
ga_instance = pygad.GA(
    ...
    #gene_type=[int] * 87,  # 每个基因都是 int 类型,
    gene_type=[int] * 82,  # 每个基因都是 int 类型,
...
print("GA instance run completed")
...
if __name__ == "__main__":
    ...
    ga_instance.save(f'/Users/katerina/Documents/GAoptimizedCNN/train_3rd/ga_instance.p')

    # Decode the best solution to get hyperparameters
    best_num_kernel_conv1, best_num_kernel_conv2, best_kernel_size_conv1, best_kernel_size_conv2, best_kernel_size_pooling = decode_gene(best_solution)
    print("Best num_kernel_conv1 : ", best_num_kernel_conv1)
    print("Best num_kernel_conv2 : ", best_num_kernel_conv2)
    print("Best kernel_size_conv1 : ", best_kernel_size_conv1)
    print("Best kernel_size_conv2 : ", best_kernel_size_conv2)
    print("Best kernel_size_pooling : ", best_kernel_size_pooling)

    # 保存最终最优解的模型和训练图像
    best_model, _, loss_value, accuracy_in_train, accuracy_in_test = train_and_eval(
        best_num_kernel_conv1,
        best_num_kernel_conv2,
        best_kernel_size_conv1,
        best_kernel_size_conv2,
        best_kernel_size_pooling,
        generation="final",
        individual="best",
        print_accuracy=True,
        return_history=True
    ...
    # Save the best model
    torch.save(best_model.state_dict(), f'/Users/katerina/Documents/GAoptimizedCNN/train_3rd/best_model.pth')
    ...
    plt.legend()
    plt.savefig(f'/Users/katerina/Documents/GAoptimizedCNN/train_3rd/final_best_model_accuracy.png')
    plt.close()

    # 保存最优解的训练损失值图像
    plt.figure()
    plt.plot(range(1, len(loss_value) + 1), loss_value, label='Train Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Final Best Model Loss')
    plt.legend()
    plt.savefig(f'/Users/katerina/Documents/GAoptimizedCNN/train_3rd/final_best_model_loss.png')
    plt.close()
